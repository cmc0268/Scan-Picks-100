...
def filter_geon_all(results):
    """단계별 '건(乾·양양양)' 종목을 개수 제한 없이 전부 반환 (단계 필터용).
    표시 순서: 단계 묶음(1→6→2→5→3→4) → 점수."""
    GEON = "양양양"
    geon = [r for r in results
            if r.get("stage", 0) in (1, 2, 3, 4, 5, 6)
            and gua_key(r.get("gua")) == GEON]
    grp = {1: 0, 6: 1, 2: 2, 5: 3, 3: 4, 4: 5}
    geon.sort(key=lambda x: (grp.get(x["stage"], 9), _pick_score(x)))
    return geon


def filter_riri(results):
    """4단계 '리(離)' + MACD 상승추세 + 5일선 상승추세. 개수 제한 없이 전부."""
